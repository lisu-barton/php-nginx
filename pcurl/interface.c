...
int  le_pcurl;
int  le_pcurl_multi_handle;
int  le_pcurl_share_handle;

_pcurl_pool *_pool;
#if defined(ZTS)
	MUTEX_T *pcurl_pool_lock;
#endif

#ifdef PHP_CURL_NEED_OPENSSL_TSL /* {{{ */
static MUTEX_T *php_pcurl_openssl_tsl = NULL;

static void php_pcurl_ssl_lock(int mode, int n, const char * file, int line)
{
	if (mode & CRYPTO_LOCK) {
		tsrm_mutex_lock(php_pcurl_openssl_tsl[n]);
	} else {
		tsrm_mutex_unlock(php_pcurl_openssl_tsl[n]);
	}
}

static unsigned long php_pcurl_ssl_id(void)
{
...
static int php_pcurl_ssl_mutex_create(void **m)
{
...
static int php_pcurl_ssl_mutex_destroy(void **m)
{
	tsrm_mutex_free(*((MUTEX_T *) m));
	return SUCCESS;
}

static int php_pcurl_ssl_mutex_lock(void **m)
{
	return tsrm_mutex_lock(*((MUTEX_T *) m));
}

static int php_pcurl_ssl_mutex_unlock(void **m)
{
	return tsrm_mutex_unlock(*((MUTEX_T *) m));
}

static struct gcry_thread_cbs php_pcurl_gnutls_tsl = {
	GCRY_THREAD_OPTION_USER,
	NULL,
	php_pcurl_ssl_mutex_create,
	php_pcurl_ssl_mutex_destroy,
	php_pcurl_ssl_mutex_lock,
	php_pcurl_ssl_mutex_unlock
};
#endif
/* }}} */

static void _php_pcurl_close_ex(php_pcurl *ch);
static void _php_pcurl_close(zend_resource *rsrc);
static void _pcurl_pool_ctor();
static void _pcurl_pool_inc();
static pcurl_node* _pcurl_node_ctor(CURL *cp);
static void _pcurl_pool_append(CURL *cp);
static CURL* _pcurl_curl_init(int seq);
static php_pcurl* _pcurl_fetch_one();
static void _pcurl_pool_dtor();
static void _php_pcurl_idle(zend_resource *rsrc);
...
# define php_pcurl_ret(__ret) RETVAL_FALSE; return __ret;
#else
# define php_pcurl_ret(__ret) RETVAL_FALSE; return;
#endif

static int php_pcurl_option_str(php_pcurl *ch, zend_long option, const char *str, const int len, zend_bool make_copy)
{
...
static int php_pcurl_option_url(php_pcurl *ch, const char *url, const int len) /* {{{ */
{
	...
	return php_pcurl_option_str(ch, CURLOPT_URL, url, len, 0);
}
/* }}} */
...
ZEND_BEGIN_ARG_INFO(arginfo_curl_which_one, 0)
	ZEND_ARG_INFO(0, ch)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_INFO_EX(arginfo_curl_fetch_one, 0, 0, 0)
	// ZEND_ARG_INFO(0, url)
ZEND_END_ARG_INFO()
...
/* {{{ pcurl_functions[]
 */
const zend_function_entry pcurl_functions[] = {
	// PHP_FE(pcurl_init,                arginfo_curl_init)
	PHP_FE(pcurl_fetch_one,			  arginfo_curl_fetch_one)
	PHP_FE(pcurl_which_one,			  arginfo_curl_which_one)
	PHP_FE(pcurl_copy_handle,         arginfo_curl_copy_handle)
	PHP_FE(pcurl_version,             arginfo_curl_version)
	PHP_FE(pcurl_setopt,              arginfo_curl_setopt)
	PHP_FE(pcurl_setopt_array,        arginfo_curl_setopt_array)
	PHP_FE(pcurl_exec,                arginfo_curl_exec)
	PHP_FE(pcurl_getinfo,             arginfo_curl_getinfo)
	PHP_FE(pcurl_error,               arginfo_curl_error)
	PHP_FE(pcurl_errno,               arginfo_curl_errno)
	PHP_FE(pcurl_close,               arginfo_curl_close)
#if LIBCURL_VERSION_NUM >= 0x070c00 /* 7.12.0 */
	PHP_FE(pcurl_strerror,            arginfo_curl_strerror)
	PHP_FE(pcurl_multi_strerror,      arginfo_curl_multi_strerror)
#endif
#if LIBCURL_VERSION_NUM >= 0x070c01 /* 7.12.1 */
	PHP_FE(pcurl_reset,               arginfo_curl_reset)
#endif
#if LIBCURL_VERSION_NUM >= 0x070f04 /* 7.15.4 */
	PHP_FE(pcurl_escape,              arginfo_curl_escape)
	PHP_FE(pcurl_unescape,            arginfo_curl_unescape)
#endif
#if LIBCURL_VERSION_NUM >= 0x071200 /* 7.18.0 */
	PHP_FE(pcurl_pause,               arginfo_curl_pause)
#endif
	PHP_FE(pcurl_multi_init,          arginfo_curl_multi_init)
	PHP_FE(pcurl_multi_add_handle,    arginfo_curl_multi_add_handle)
	PHP_FE(pcurl_multi_remove_handle, arginfo_curl_multi_remove_handle)
	PHP_FE(pcurl_multi_select,        arginfo_curl_multi_select)
	PHP_FE(pcurl_multi_exec,          arginfo_curl_multi_exec)
	PHP_FE(pcurl_multi_getcontent,    arginfo_curl_multi_getcontent)
	PHP_FE(pcurl_multi_info_read,     arginfo_curl_multi_info_read)
	PHP_FE(pcurl_multi_close,         arginfo_curl_multi_close)
#if LIBCURL_VERSION_NUM >= 0x070f04 /* 7.15.4 */
	PHP_FE(pcurl_multi_setopt,        arginfo_curl_multi_setopt)
#endif
	PHP_FE(pcurl_share_init,          arginfo_curl_share_init)
	PHP_FE(pcurl_share_close,         arginfo_curl_share_close)
	PHP_FE(pcurl_share_setopt,        arginfo_curl_share_setopt)
	PHP_FE(pcurl_file_create,         arginfo_curlfile_create)
	PHP_FE_END
};
/* }}} */
...
PHP_MINIT_FUNCTION(pcurl)
{
	...
	le_pcurl = zend_register_list_destructors_ex(_php_pcurl_idle, _php_pcurl_close, "pcurl", module_number);
	le_pcurl_multi_handle = zend_register_list_destructors_ex(_php_pcurl_multi_close, NULL, "pcurl_multi", module_number);
	le_pcurl_share_handle = zend_register_list_destructors_ex(_php_pcurl_share_close, NULL, "pcurl_share", module_number);
...
// curl未注册常量，说明未安装curl
if( zend_get_constant_str("CURLOPT_URL", 11) == NULL ){
	...
	if (!CRYPTO_get_id_callback()) {
		...
		php_pcurl_openssl_tsl = malloc(c * sizeof(MUTEX_T));
		if (!php_pcurl_openssl_tsl) {
			return FAILURE;
		}

		for (i = 0; i < c; ++i) {
			php_pcurl_openssl_tsl[i] = tsrm_mutex_alloc();
		}

		CRYPTO_set_id_callback(php_pcurl_ssl_id);
		CRYPTO_set_locking_callback(php_pcurl_ssl_lock);
	}
#endif
#ifdef PHP_CURL_NEED_GNUTLS_TSL
	gcry_control(GCRYCTL_SET_THREAD_CBS, &php_pcurl_gnutls_tsl);
#endif
...
PHP_MSHUTDOWN_FUNCTION(pcurl)
{
	...
	if (php_pcurl_openssl_tsl) {
		...
		for (i = 0; i < c; ++i) {
			tsrm_mutex_free(php_pcurl_openssl_tsl[i]);
		}

		free(php_pcurl_openssl_tsl);
		php_pcurl_openssl_tsl = NULL;
	}
#endif
...
/* {{{ curl_write_nothing
 * Used as a work around. See _php_pcurl_close_ex
 */
...
{
	php_pcurl *ch = (php_pcurl *) ctx;
	php_pcurl_write *t = ch->handlers->write;
	...
	switch (t->method) {
		...
		case PHP_CURL_USER: {
			...
			if (error == FAILURE) {
				...
				_php_pcurl_verify_handlers(ch, 1);
...
{
	php_pcurl *ch = (php_pcurl *) ctx;
	php_pcurl_fnmatch *t = ch->handlers->fnmatch;
...
{
	php_pcurl *ch = (php_pcurl *)clientp;
	php_pcurl_progress *t = ch->handlers->progress;
	size_t	rval = 0;
...
{
	php_pcurl *ch = (php_pcurl *)ctx;
	php_pcurl_read *t = ch->handlers->read;
	int length = 0;
...
{
	php_pcurl *ch = (php_pcurl *) ctx;
	php_pcurl_write *t = ch->handlers->write_header;
	size_t length = size * nmemb;
...
/* {{{ alloc_curl_handle
 */
static php_pcurl *alloc_curl_handle()
{
	php_pcurl *ch               = ecalloc(1, sizeof(php_pcurl));
	ch->to_free                = ecalloc(1, sizeof(struct _php_pcurl_free));
	ch->handlers               = ecalloc(1, sizeof(php_pcurl_handlers));
	ch->handlers->write        = ecalloc(1, sizeof(php_pcurl_write));
	ch->handlers->write_header = ecalloc(1, sizeof(php_pcurl_write));
	ch->handlers->read         = ecalloc(1, sizeof(php_pcurl_read));
	...
	memset(&ch->err, 0, sizeof(struct _php_pcurl_error));
...
/* {{{ proto resource pcurl_getone([string url])
   get a cURL session from pool */
PHP_FUNCTION(pcurl_which_one)
{
	...
	if ((ch = (php_pcurl*)zend_fetch_resource(Z_RES_P(zid), le_pcurl_name, le_pcurl)) == NULL) {
		RETURN_FALSE;
	}

	_php_pcurl_cleanup_handle(ch);

	sprintf(str, "php_pcurl:%x, CURL:%x", ch, ch->cp);
...
/* {{{ _php_pcurl_set_default_options()
   Set default options for a handle */
static void _php_pcurl_set_default_options(php_pcurl *ch)
{
	char *cainfo;
...
/* {{{ _pcurl_set_default_options()
   Set default options for a handle */
static void _pcurl_set_default_options(CURL *cp)
{
	char *cainfo;
	...
	curl_easy_setopt(cp, CURLOPT_READFUNCTION,      curl_read);
	curl_easy_setopt(cp, CURLOPT_HEADERFUNCTION,    curl_write_header);
	// curl_easy_setopt(cp, CURLOPT_ERRORBUFFER,       ch->err.str);
	// curl_easy_setopt(cp, CURLOPT_FILE,              (void *) ch);
	// curl_easy_setopt(cp, CURLOPT_INFILE,            (void *) ch);
	// curl_easy_setopt(cp, CURLOPT_WRITEHEADER,       (void *) ch);
	curl_easy_setopt(cp, CURLOPT_DNS_USE_GLOBAL_CACHE, 1);
	curl_easy_setopt(cp, CURLOPT_DNS_CACHE_TIMEOUT, 120);
	curl_easy_setopt(cp, CURLOPT_MAXREDIRS, 20); /* prevent infinite redirects */
	...
	if (cainfo && strlen(cainfo) > 0) {
		curl_easy_setopt(cp, CURLOPT_CAINFO, cainfo);
	}
...
PHP_FUNCTION(pcurl_fetch_one)
{
	...
	ZVAL_RES(return_value, zend_register_resource(ch, le_pcurl));
...
PHP_FUNCTION(pcurl_init)
{
	...
	_php_pcurl_set_default_options(ch);

	if (url) {
		if (php_pcurl_option_url(ch, url, url_len) == FAILURE) {
			_php_pcurl_close_ex(ch);
			RETURN_FALSE;
		}
	}
...
PHP_FUNCTION(pcurl_copy_handle)
{
	...
	if (ch->handlers->progress) {
		dupch->handlers->progress = ecalloc(1, sizeof(php_pcurl_progress));
...
/* Available since 7.21.0 */
	...
	if (ch->handlers->fnmatch) {
		dupch->handlers->fnmatch = ecalloc(1, sizeof(php_pcurl_fnmatch));
	...
	ZVAL_RES(return_value, zend_register_resource(dupch, le_pcurl));
	dupch->res = Z_RES_P(return_value);
}
/* }}} */

static int _php_pcurl_setopt(php_pcurl *ch, zend_long option, zval *zvalue) /* {{{ */
{
	CURLcode error = CURLE_OK;
	zend_long lval;
	...
	switch (option) {
...
#endif
		{
			...
			int ret = php_pcurl_option_str(ch, option, ZSTR_VAL(str), ZSTR_LEN(str), 0);
		...
		{
			zend_string *str = zval_get_string(zvalue);
			int ret = php_pcurl_option_str(ch, option, ZSTR_VAL(str), ZSTR_LEN(str), 1);
			zend_string_release(str);
			return ret;
		}
		...
		{
			zend_string *str = zval_get_string(zvalue);
			int ret = php_pcurl_option_url(ch, ZSTR_VAL(str), ZSTR_LEN(str));
			zend_string_release(str);
			return ret;
		}
			...
			if (Z_TYPE_P(zvalue) == IS_ARRAY || Z_TYPE_P(zvalue) == IS_OBJECT) {
				...
				ZEND_HASH_FOREACH_KEY_VAL(postfields, num_key, string_key, current) {
					...
					if (Z_TYPE_P(current) == IS_OBJECT &&
							instanceof_function(Z_OBJCE_P(current), curl_pCURLFile_class)) {
						/* new-style file upload */
						zval *prop, rv;
						char *type = NULL, *filename = NULL;

						prop = zend_read_property(curl_pCURLFile_class, current, "name", sizeof("name")-1, 0, &rv);
						if (Z_TYPE_P(prop) != IS_STRING) {
							...
							prop = zend_read_property(curl_pCURLFile_class, current, "mime", sizeof("mime")-1, 0, &rv);
							if (Z_TYPE_P(prop) == IS_STRING && Z_STRLEN_P(prop) > 0) {
								type = Z_STRVAL_P(prop);
							}
							prop = zend_read_property(curl_pCURLFile_class, current, "postname", sizeof("postname")-1, 0, &rv);
							if (Z_TYPE_P(prop) == IS_STRING && Z_STRLEN_P(prop) > 0) {
		...
		case CURLOPT_PROGRESSFUNCTION:
			...
			if (ch->handlers->progress == NULL) {
				ch->handlers->progress = ecalloc(1, sizeof(php_pcurl_progress));
...
#endif
		{
			...
			ret = php_pcurl_option_str(ch, option, ZSTR_VAL(str), ZSTR_LEN(str), 0);
			zend_string_release(str);
			return ret;
		}
		...
		case CURLOPT_SHARE:
			{
				php_pcurlsh *sh;
				if ((sh = (php_pcurlsh *)zend_fetch_resource_ex(zvalue, le_pcurl_share_handle_name, le_pcurl_share_handle))) {
		...
		case CURLOPT_FNMATCH_FUNCTION:
			...
			if (ch->handlers->fnmatch == NULL) {
				ch->handlers->fnmatch = ecalloc(1, sizeof(php_pcurl_fnmatch));
...
PHP_FUNCTION(pcurl_setopt)
{
	...
	if (_php_pcurl_setopt(ch, options, zvalue) == SUCCESS) {
		RETURN_TRUE;
	} else {
		RETURN_FALSE;
	}
...
PHP_FUNCTION(pcurl_setopt_array)
{
	...
	ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(arr), option, string_key, entry) {
		...
		if (_php_pcurl_setopt(ch, (zend_long) option, entry) == FAILURE) {
			RETURN_FALSE;
		}
	} ZEND_HASH_FOREACH_END();
...
void _php_pcurl_cleanup_handle(php_pcurl *ch)
{
...
static void _php_pcurl_close_ex(php_pcurl *ch)
{
	...
	_php_pcurl_verify_handlers(ch, 0);
...
static void _php_pcurl_close(zend_resource *rsrc)
{
	php_pcurl *ch = (php_pcurl *) rsrc->ptr;
	_php_pcurl_close_ex(ch);
}
/* }}} */
...
static void _php_pcurl_idle(zend_resource *rsrc)
{
	...
	#if defined(ZTS)
	tsrm_mutex_lock(pcurl_pool_lock);
	#endif

	node = malloc(sizeof(pcurl_node));
	node->cp = ch->cp;
	// fprintf(stderr, "CURL: %X IDLE.\n", node->cp);

	node->next = _pool->idle_link;
	_pool->idle_link = node;

	#if defined(ZTS)
	tsrm_mutex_unlock(pcurl_pool_lock);
	#endif
...
static void _pcurl_pool_ctor()
{
	...
	_pool = malloc(sizeof(_pcurl_pool));
	_pool->all_link = NULL;
	_pool->idle_link = NULL;
	_pool->size = 0;
...
static void _pcurl_pool_inc()
{
	...
	if(_pool->size > PCURL_POOL_MAX ){
		return;
	}
	
	dest = _pool->size + PCURL_POOL_INC;
	for(i=0; i < dest; i++){
		// 第N个，索引编号为N.
		cp = _pcurl_curl_init(_pool->size + i);
		if(NULL == cp){
			return;
		}

		_pcurl_pool_append(cp);

		_pool->size++;
		// printf(" pool initializing, seq: %d, size: %d.", i, _pool->size);
		// fprintf(stderr, " pool initializing, seq: %d, size: %d.\n", i, _pool->size);
	}
}
...
static void _pcurl_pool_append(CURL *cp)
{
	...
	if(NULL == _pool->all_link){
		_pool->all_link = _pcurl_node_ctor(cp);
		_pool->idle_link = _pcurl_node_ctor(cp);
		return;
	}
	...
	tail->next = _pcurl_node_ctor(cp);

	// 增加到空闲列表的尾部
	tail = _pool->idle_link;
	while(NULL != tail->next){
		tail = tail->next;
	}
	tail->next = _pcurl_node_ctor(cp);
...
static CURL* _pcurl_curl_init(int seq)
{
    ...
    curl_easy_setopt(cp, CURLOPT_TCP_KEEPALIVE, 1L);
            //从php.init中读取配置，默认为1800秒
    zend_long curl_keepidle = zend_ini_long("pcurl.tcp_keepidle", 18, 1800);
    if (curl_keepidle < 0){
            curl_keepidle = 1800;
    }
...
static php_pcurl* _pcurl_fetch_one()
{
	...
	if(NULL == _pool->idle_link){
		// 资源不够，进行增长
		_pcurl_pool_inc();
	}

	while(NULL == _pool->idle_link){
		// 等待有新的空闲
		// sleep();
	...
	node = _pool->idle_link;
	_pool->idle_link = node->next;
	node->next=NULL;
...
static void _pcurl_pool_dtor(){
	...
	while(node != NULL){	
		...
		curl_easy_setopt(node->cp, CURLOPT_HEADERFUNCTION, curl_write_nothing);
		curl_easy_setopt(node->cp, CURLOPT_WRITEFUNCTION, curl_write_nothing);
		curl_easy_cleanup(node->cp);
		free(node);
		node = node_next;
	}
...
#if LIBCURL_VERSION_NUM >= 0x070c01 /* 7.12.1 */
/* {{{ _php_pcurl_reset_handlers()
   Reset all handlers of a given php_pcurl */
static void _php_pcurl_reset_handlers(php_pcurl *ch)
{
	if (!Z_ISUNDEF(ch->handlers->write->stream)) {
		zval_ptr_dtor(&ch->handlers->write->stream);
		ZVAL_UNDEF(&ch->handlers->write->stream);
