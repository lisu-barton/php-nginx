...
PHP_ARG_ENABLE(pcurl, whether to enable pcurl support,
Make sure that the comment is aligned:
[  --enable-pcurl           Enable pcurl support])

if test "$PHP_PCURL" != "no"; then
  if test -r $PHP_PCURL/include/curl/easy.h; then
    CURL_DIR=$PHP_PCURL
  else
  ...
  AC_MSG_CHECKING(for cURL 7.10.5 or greater - $CURL_DIR -)
...
int main(int argc, char *argv[])
{
  ...
  PHP_NEW_EXTENSION(pcurl, interface.c multi.c share.c curl_file.c, $ext_shared)
