...
PHP_FUNCTION(pcurl_multi_init)
{
	...
	mh = ecalloc(1, sizeof(php_pcurlm));
	mh->multi = curl_multi_init();

	zend_llist_init(&mh->easyh, sizeof(zval), _php_pcurl_multi_cleanup_list, 0);

	RETURN_RES(zend_register_resource(mh, le_pcurl_multi_handle));
}
/* }}} */
...
PHP_FUNCTION(pcurl_multi_add_handle)
{
	...
	if ((mh = (php_pcurlm *)zend_fetch_resource(Z_RES_P(z_mh), le_pcurl_multi_handle_name, le_pcurl_multi_handle)) == NULL) {
		RETURN_FALSE;
	}

	if ((ch = (php_pcurl *)zend_fetch_resource(Z_RES_P(z_ch), le_pcurl_name, le_pcurl)) == NULL) {
		RETURN_FALSE;
	}
...
PHP_FUNCTION(pcurl_multi_exec)
{
...

	{
		...
		for (pz_ch = (zval *)zend_llist_get_first_ex(&mh->easyh, &pos); pz_ch;
			...
			if ((ch = (php_pcurl *)zend_fetch_resource(Z_RES_P(pz_ch), le_pcurl_name, le_pcurl)) == NULL) {
				RETURN_FALSE;
			}
	...
	/* find the original easy curl handle */
	{
		...
		for(pz_ch = (zval *)zend_llist_get_first_ex(&mh->easyh, &pos); pz_ch;
			...
			if (ch->cp == tmp_msg->easy_handle) {
				...
				/* we are adding a reference to the underlying php_pcurl
...
void _php_pcurl_multi_close(zend_resource *rsrc) /* {{{ */
{
	php_pcurlm *mh = (php_pcurlm *)rsrc->ptr;
	if (mh) {
		zend_llist_position pos;
		php_pcurl *ch;
		zval	*pz_ch;
		...
		for (pz_ch = (zval *)zend_llist_get_first_ex(&mh->easyh, &pos); pz_ch;
			...
			if (Z_RES_P(pz_ch)->ptr) {
				if ((ch = (php_pcurl *) zend_fetch_resource(Z_RES_P(pz_ch), le_pcurl_name, le_pcurl))) {
...
static int _php_pcurl_multi_setopt(php_pcurlm *mh, zend_long option, zval *zvalue, zval *return_value) /* {{{ */
{
...
PHP_FUNCTION(pcurl_multi_setopt)
{
	...
	if (!_php_pcurl_multi_setopt(mh, options, zvalue, return_value)) {
		RETURN_TRUE;
	} else {
		RETURN_FALSE;
	}
